...
static main(void) {
	...
	FindAddress("client_class_table", "48 8D 15 ?? ?? ?? ?? 48 89 6C 24 ?? 4C 8D 0D ?? ?? ?? ?? 48 89 54 24 ?? 4C 8D 05 ?? ?? ?? ?? 48 8B D6 C7 44 24 ?? ?? ?? ?? ?? 48 8B CF FF 90 ?? ?? ?? ?? 84 C0 75 1A EB 20 83 FB 03"); //global variable
